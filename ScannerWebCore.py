...
class Scanner_Web_Core (object):
	...
	def buildArgvSys(self):
		...
		self.parser.add_option("-d","--dork", dest="dork",help="Look for dork", metavar="DORK",type="str",callback=self.listExist,action="callback")
		self.parser.add_option("--pages",help="Set number of pages to get from the engin",dest="pages",type="int",default=1)
		self.parser.add_option("-e","--engin",dest="engin",help="Set engin to search for dork", metavar="ENGIN",default="google")
		...
		self.parser.add_option("--udp-ports",dest="udp-ports", help="Scann udp ports in the server",default=False, action="store_true")
		...
		self.parser.add_option("--sql-injection",dest="sql",help="scann for sql injection error",default=False,action="store_true")
	...
	def getDorkResults (self,html):
		...
		engin = self.options.get("engin").lower()
		if engin == "bing":
			result = re.findall(r'<h2><a href="(.*?)" h="',html)
		elif engin == "google":
			result = re.findall(r'<h3 class="r"><a href="(.*?)"', html)
	...
	Method listExist(self,option,opt_str,value,parser):
		...
		if it is file the method will check if is exist or not an return error in if is not exist 
	"""
	def listExist(self,option,opt_str,value,parser):
		...
		if self.regex.isPath(value):
			try :
				...
				raise OptionValueError("The list passed in argument "+opt_str+" not found")
		else :
			if option.dest == "dork":
	...
	def updateChecker(self):
		getVersion = re.findall(r"<!-- Version (\d+\.\d+) -->",str(self.connector.connection("https://raw.githubusercontent.com/SuicV/Scanner-web/master/README.md").response.read()))[0]
