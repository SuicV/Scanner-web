...
class connector (object):
	...
	def checkProxy(self,proxy):
		try :
			Request = request.Request("https://www.github.com",headers={"User-Agent":self.USER_AGENT[0]})
			parser = self.parser(proxy)
			Request.set_proxy(parser.netloc,parser.scheme)
	...
	def tcpScan (self, sc, target):
		...
		for port in self.ports :
			...
			if socketTcp.connect_ex((target, port)) is 0 :
	...
	def udpScan (self, sc, target):
		...
		for port in self.ports :
			...
			if socketUdp.connect_ex((target, port)) is 0 :
				openedTcp.append(port)
